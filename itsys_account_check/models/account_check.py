...
class account_check(models.Model):
    ...
    state= fields.Selection([('draft_collect', 'New'),
                              ...
                              copy=False, )
    supplier_id=fields.Many2one(comodel_name='res.partner', string='Hashed to Supplier')
    ...
    def check_move_create(self,debit,credit):
        ...
        if self.journal_id.journal_state=='receive':
            ...
            created_journal=account_move_obj.create({'name': self.name, 'ref': self.ref,
                                     'journal_id': self.journal_id.id,
                                     'line_ids': [(0, 0, {
                                         'name': self.name, 'account_id': self.property_account_receivable_id.id,
