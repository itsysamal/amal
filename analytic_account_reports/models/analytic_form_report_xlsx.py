        ...
        if all_account_expense_collection:
            ...
            for exp in all_account_expense_collection:
                ...
                if values_exp:
                    ...
                    worksheet.write(row, col + 1, ' ', header2_format)
                row += 1
            worksheet.write(row, col + 1, total_total_cost_revenue, header2_format)
            worksheet.write(row + 1, col + 1, total_total_cost_per_qty, header2_format)
            worksheet.write(row + 2, col + 1, total_total_revenue_income_other, header2_format)
            worksheet.write(row + 3, col + 1, total_total_profit, header2_format)
            worksheet.write(row + 4, col + 1, total_total_qty_result, header2_format)
            worksheet.write(row + 5, col + 1, total_total_balance, header2_format)
        else:
            worksheet.write(row + 5, col + 1, '%.2f' % total_total_cost_revenue, header2_format)
            worksheet.write(row + 6, col + 1, '%.2f' % total_total_cost_per_qty, header2_format)
            worksheet.write(row + 7, col + 1, '%.2f' % total_total_revenue_income_other, header2_format)
            worksheet.write(row + 8, col + 1, '%.2f' % total_total_profit, header2_format)
            worksheet.write(row + 9, col + 1, '%.2f' % total_total_qty_result, header2_format)
            worksheet.write(row + 10, col + 1, '%.2f' % total_total_balance, header2_format)
        row = 4
        worksheet.set_row(row + 4, 30)
        worksheet.set_row(row + 8, 30)
